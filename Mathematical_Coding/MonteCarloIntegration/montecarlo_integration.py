...
def ball_volume(n, N=10000):
    ...
    """
    #Get a uniform distribution of points
    points = np.random.uniform(-1,1,(n,N))                                     
    lengths = la.norm(points, axis=0)
    #Check to see which values are less than 1
    num_within = np.count_nonzero(lengths < 1)
    volume = 2**n
    #Return the estimated volume
    return(volume*(num_within/N))
...
def mc_integrate1d(f, a, b, N=10000):
    ...
    """
    #Use the algorithm that is given in the book using a uniform distribution
...
def mc_integrate(f, mins, maxs, N=10000):
    ...
    """
    #Reshape and turn the maxs and mins into numpy arrays
    maxs = np.reshape(np.array(maxs), (len(maxs),1))
    mins = np.reshape(np.array(mins), (len(mins),1))
    #Calculate the volume of the box and sample
    vol = np.prod(maxs-mins)
    x = np.random.uniform(0,1,(len(mins),N))
    x = x*(maxs-mins) + mins
    #Calculate the function at each of the sampled points and calculate the approximate integral
    ys = [f(x[:,i]) for i in range(N)]
    integ = 1/N*vol*np.sum(ys)
    return(float(integ))
...
def prob4():
    ...
    """
    #Define the function and axis of integration
    f = lambda x: 1/(2*np.pi)**(2)*np.exp((-1*x.T@x)/2)
    mins = [-3/2,0,0,0]
    maxs = [3/4,1,1/2,1]
    means, cov = np.zeros(4), np.eye(4)
    #Get the exact value using scipy
    exact = stats.mvn.mvnun(np.array(mins), np.array(maxs), means, cov)[0]
    N = np.logspace(1,5,20)
    my_err = []
    #Calculate the error using different values of N and Plot
    for n in N:
        my_err.append(np.abs(exact-mc_integrate(f,mins, maxs,int(n)))/np.abs(exact))
    plt.loglog(N, my_err, label="Relative Error")
    plt.loglog(N, 1/N**(1/2), label = "1/N**(1/2)")
    plt.title('Monte Carlo Error')
    #Label all of the axis and use a legend
    plt.xlabel('N Value')
    plt.ylabel('Error')
    plt.legend()
    plt.show()
